...
void DX12SwapChain::CreateSwapChain(IDXGIFactory4 *a_dxgiFactory, DXGI_SWAP_CHAIN_DESC a_swapChainDesc)
{
    ...
    // needs a Streamline-owned present path. Which of the two conditions produced
    ...
    logger::info("[DX12SwapChain] Creating D3D12 proxy swap chain {}x{} fmt={} flags=0x{:X} backend={} "
                 ...
                 upscaling->UsesFSRFrameGeneration(), fidelityFX->module != nullptr);

    const auto createNativeSwapChain = [&]() {
        // Under eUseManualHooking Streamline interposes nothing on its own, so a
        // swap chain created from the raw factory is invisible to it -- and DLSS-G
        // interpolates inside the swap chain's Present, so it could never generate
        // a frame. Upgrade the factory first when DLSS-G is the backend; the call
        // is a no-op for every other configuration, and on failure we keep the
        // native factory and simply present without generation.
        ...
        DX::ThrowIfFailed(presentFactory->CreateSwapChainForHwnd(commandQueue.get(), a_swapChainDesc.OutputWindow,
