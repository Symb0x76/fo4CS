...
RuntimeAdapter::RuntimeAdapter() noexcept {
  ...
  capabilities = {.flavor = RuntimeFlavor::kPreNG,
                  .name = "PreNG",
                  // PreNG supports Streamline. This was false on the belief that
                  // 1.10.163 had an engine defect; measured in game, DLSS, DLSS-G and
                  // Reflex all initialise and run there. The two things that actually
                  // broke were a loader-lock deadlock (see Streamline::EnsureLoaded)
                  // and RenderDoc suppressing NVAPI in the process, which makes NGX
                  // report the GPU unsupported.
                  .supportsStreamline = true,
                  .supportsD3D12Proxy = true,
                  .requiresExistingRendererRecovery = false,
                  .supportsDeferredPipeline = true};
