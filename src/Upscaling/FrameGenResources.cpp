...
void Upscaling::CreateFrameGenerationResources()
{
	...
	for (int index = 0; index < 2; index++) {
		...
		// The probe therefore never fired and the buffer silently inherited kMain's
		// R11G11B10_FLOAT (group 5) against an R8G8B8A8_UNORM backbuffer (group 2). The
		// comment #21 left behind, claiming kFrameBuffer is R11G11B10_FLOAT, described a
		// measurement that never happened; the run log's post-display source is fmt 28.
		//
		// Ported from codex/cs-refactor 7f1fc6f, where it also carries an FFX-side net
		// (5d276af, 4d1121b) that this branch does not have. That net is not load-bearing
