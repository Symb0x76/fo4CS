...
void Upscaling::PostAlpha()
{
...

	{
		...
		// kMainTemp across this hook and feeding the result to the UI-layer shader as a
		// forced alpha. It has been removed: the crosshair is not a native draw, and the
		// call site this hook patches is a whole-view BSShaderAccumulator flush rather
		// than a crosshair draw. The UI layer no longer comes from differencing at all --
		// see CaptureUIColorAndAlphaResource below. What remains in this function --
		// generateSharedBuffersCS above -- produces the shared motion-vector and depth
		// buffers and is load-bearing for frame generation, so the hook that calls us
		// must stay.
	}
}
