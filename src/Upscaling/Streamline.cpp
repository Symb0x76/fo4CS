...
// f4se.log never reaches "loaded correctly".
//
...
// always saw initialized == false and skipped, leaving DLSS-G with an
// un-upgraded device, an un-proxied command queue and no Streamline present
// path. Everything after the loader lock is released is fair game, and device
// creation is well past it, so callers may now pull the load forward.
...
bool Streamline::UpgradeDXGIFactoryForDLSSG(IDXGIFactory4** factory)
{
	...
	if (result != sl::Result::eOk || !upgradedInterface) {
		...
		LogEvent(Event::Error, "[Streamline] slUpgradeInterface failed for DXGI factory: {}", ResultToString(result));
