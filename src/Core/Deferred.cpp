...
// The pipeline trace, the GBuffer dump and the shared LightingDrawState now live
// in src/Core/Deferred/DeferredTrace.cpp. Hoisting the namespace here keeps all
// seventeen TraceDeferred call sites, the OM-state helpers and the thread_local
// declaration spelled exactly as they were.
...
bool Deferred::BeginLightingDraw(ID3D11DeviceContext* a_context)
{
	...
	for (const auto& binding : GetDeferredRenderTargetBindings()) {
	...
	if (!loggedReady.exchange(true, std::memory_order_relaxed)) {
		logger::info(
			...
			GetDeferredRenderTargetBindings().size(),
